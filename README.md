...
как правило, стоят мегакорпорации, как пример _WhatsUp_ (***Meta***) или _Viber_ (***Rakuten***). 

Зачастую, компании принимают решение использовать тот или иной мессенджер, как стандарт де-факто для оперативного решения рабочих вопросов, связи со своими сотрудниками. 
Начинается бесконтрольный процесс появления рабочих чатов. Нередко, к части из них администрация подключает сторонних ботов, выдавая им таким образом доступ ко всей перписке внутри данного чата.
Спецслужбы различных стран, под благими предлогами, начинают давить на владельцев крупных мессенджеров, с целью получить доступ к переписке всех их пользователей.
...
в Париже 24 августа 2024 года, ранее - попытки блокирования _Telegram_ в России.

Протокол [_XMPP_](https://xmpp.org/about/technology-overview/) позволяет использовать один из тысяч бесплатных серверов в сети интернет или поднять свой сервер, который, благодаря децентрализованности системы, 
может быть как доступным в любой точке земного шара, так и изолированным от внешнего мира в соответствии с корпоративными стандартами, принятыми в организации. Архитектура сети XMPP похожа на электронную почту; 
в результате каждый может запустить свой собственный сервер XMPP, что позволяет отдельным лицам и организациям контролировать свой коммуникационный опыт.

Защита с использованием SASL и TLS встроена в основные  [спецификации](https://xmpp.org/rfcs/) XMPP . Кроме того, сообщество разработчиков XMPP активно работает над сквозным шифрованием, чтобы еще больше поднять планку безопасности.

Здесь мы рассмотрим разворачивание стенда с отказоустойчивой реализацией сервиса _XMPP_, готового обслуживать какой-либо офис или организацию.
