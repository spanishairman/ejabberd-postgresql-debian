...
Для изолированной сети явно задаётся имя сети, IP-адрес и маска. Параметр _libvirt\_\_always\_destroy_ со значением "true" не разрешает виртуальным машинам, которые используют сеть, но не создавали её уничтожать её при уничтожении виртуальной машины (домена).
