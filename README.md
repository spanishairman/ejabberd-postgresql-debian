...
Второй интерфейс у них находится в сети 192.168.1.0/29, которую мы также условно, будем считать демилитаризованной зоной. Внутренняя сеть - 192.168.1.8/29, в которой размещены т.н. Backend-серверы,
отделена от демилитаризованной зоны виртуальным хостом _gw1server_ с двумя сетевыми интерфейсами с адресами 192.168.1.1/29 и 192.168.1.9.29, 
...
Затем количество резервных копий в томе достигнет максимального значения (параметр ***Maximum Volume Jobs = 7***) и следующая задача запишет новую копию в новый том.
...
у которого истёк срок, заданный параметром ***Volume Retention = 21  days*** (отсчитывается от времени последней записи в том). 
...
данный том будет усечён до нулевого размера.
