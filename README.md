...
которым будет доверять клиентское ПО - браузер или xmpp-клиент. Для этих целей отлично подойдет __xca__ - [приложение](https://hohnstaedt.de/xca/), предназначеное для создания 
...
в специальное хранилище __Доверенные корневые центры сертификации__ оснастки __Сертификаты__.

В свойствах выпускаемых сертификатов заполним параметр _Альтернативное имя субъекта_ так, чтобы он содержал как доменные имена сервера, 
...
```
Поэтому потребуется последовательно остановить все процессы _beam.smp_, что остановит сервисный процесс _ejabberd.service_, остановить сервис _epmd_, после чего запустить _ejabberd.service_. Пример соответствующей задчи для _Ansible_:
```
